...
(function () {
  ...
  const state = {
    ...
    exportQuality: 0.92,
    exportScale: 1,
    zoom: 0, // 0 = auto-fit, otherwise manual zoom level (0.1 - 5)
  };
  ...
  function render() {
    ...
    if (state.aspect !== "free") {
      ...
      const availH =
        totalH - pad * 2 - frameTop - frameBottom - textTopH - textBottomH;
      ...
      const contentYBase =
        Math.round((totalH - contentH - textTopH - textBottomH) / 2) + textTopH;
    ...
    if (shadowConf) {
      ...
      const alpha = Math.round(shadowConf.opacity * 2.55)
        .toString(16)
        .padStart(2, "0");
  ...
  function fitCanvasToView() {
    ...
    // Allow scroll when zoomed in
    area.style.overflow =
      scale * cw > availW || scale * ch > availH ? "auto" : "hidden";
  ...
  // Scroll wheel zoom on canvas area (Ctrl+scroll)
  document.getElementById("canvas-area")?.addEventListener(
    "wheel",
    (e) => {
      if (!e.ctrlKey && !e.metaKey) return;
      e.preventDefault();
      const currentScale = state.zoom === 0 ? getAutoFitScale() : state.zoom;
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "=" || e.key === "+") {
      e.preventDefault();
      const s = state.zoom === 0 ? getAutoFitScale() : state.zoom;
      setZoom(s + 0.1);
    } else if (e.key === "-") {
      e.preventDefault();
      const s = state.zoom === 0 ? getAutoFitScale() : state.zoom;
  ...
  function exportImage() {
    ...
    const quality =
      state.exportFormat === "png" ? undefined : state.exportQuality;
  ...
  function setupUI() {
    ...
    document.querySelectorAll(".tab-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        document
          .querySelector(`.panel-content[data-panel="${btn.dataset.tab}"]`)
    ...
    document.getElementById("bg-direction").addEventListener("change", (e) => {
      state.bg.direction = parseInt(e.target.value);
      render();
    });
    document.getElementById("btn-solid").addEventListener("click", () => {
      state.bg.type = "solid";
      state.bg.solid = document.getElementById("bg-solid").value;
      render();
      clearGradientActive();
    });
    ...
    document.querySelectorAll(".opt-btn[data-frame]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".opt-btn[data-frame]")
          .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
    ...
    document.querySelectorAll(".opt-btn[data-aspect]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".opt-btn[data-aspect]")
          .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
    ...
    document.querySelectorAll(".opt-btn[data-shadow]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".opt-btn[data-shadow]")
          .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
    ...
    // Watermark
    document
      .getElementById("watermark-toggle")
    ...
    document.querySelectorAll(".opt-btn[data-theme]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".opt-btn[data-theme]")
          .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
    ...
    document.getElementById("code-linenums").addEventListener("change", (e) => {
      state.codeLineNums = e.target.checked;
      render();
    });
    ...
    document.querySelectorAll(".opt-btn[data-format]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".opt-btn[data-format]")
          .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
    ...
    document.querySelectorAll(".opt-btn[data-scale]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".opt-btn[data-scale]")
          .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
    ...
    document
      .getElementById("btn-export-download")
      .addEventListener("click", exportImage);
    document
      .getElementById("btn-export-copy")
      .addEventListener("click", copyToClipboard);
