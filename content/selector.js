...
(function () {
  ...
  function startElementMode() {
    ...
    mode = "element";
    showTooltip(
      "Click any element to capture · ESC to cancel · Scroll to resize selection",
    );
    ...
    document.addEventListener("wheel", onElementScroll, {
      capture: true,
      passive: false,
    });
    selectedDepth = 0;
  ...
  function onElementClick(e) {
    ...
    // Hide all PostSnap UI before capture
    cleanup();
  ...
  function onRegionEnd(e) {
    ...
    // Hide all PostSnap UI before capture so it doesn't appear in screenshot
    cleanup();
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        chrome.runtime.sendMessage({ action: "captureAndOpenEditor", crop });
  ...
  function cleanup() {
    ...
    if (hoveredEl) {
      hoveredEl.classList.remove("postsnap-hover-outline");
      hoveredEl = null;
    }
